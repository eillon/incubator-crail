...
public interface StorageServer extends Configurable, Runnable {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // init the M,w,H,d from config file, according the storageName
        double M, w, H, d;
        if (storageName.equalsIgnoreCase("org.apache.crail.storage.tcp.TcpStorageTier")) {
            M = CrailConstants.STORAGE_DRAM_MAX_IO_RATE;
            w = CrailConstants.STORAGE_DRAM_RATE_RATIO;
            H = CrailConstants.NETWORK_TCP_BANDWIDTH;
            d = CrailConstants.NETWORK_TCP_DELAY;
        } else if (storageName.equalsIgnoreCase("org.apache.crail.storage.rdma.RdmaStorageTier")) {
            M = CrailConstants.STORAGE_DRAM_MAX_IO_RATE;
            w = CrailConstants.STORAGE_DRAM_RATE_RATIO;
            H = CrailConstants.NETWORK_RDMA_BANDWIDTH;
            d = CrailConstants.NETWORK_RDMA_DELAY;
        } else if (storageName.equalsIgnoreCase("org.apache.crail.storage.nvmf.NvmfStorageTier")) {
            M = CrailConstants.STORAGE_NVM_MAX_IO_RATE;
            w = CrailConstants.STORAGE_NVM_RATE_RATIO;
            H = CrailConstants.NETWORK_RDMA_BANDWIDTH;
            d = CrailConstants.NETWORK_RDMA_DELAY;
        } else {
            LOG.error("Unknown storageName, set the M,w,H,d to 1");
            M = 1;
            w = 1;
            H = 1;
            d = 1;
        ...
        while (server.isAlive()) {
            ...
            if (resource == null) {
                ...
                storageRpc.setBlock(lba, resource.getAddress(), resource.getLength(), resource.getKey());  // maybe should change here
                ...
                sumCount += diffCount;
                LOG.info("BUILDING datanode statistics, freeBlocks " + sumCount);
            }
        }
        ...
        while (server.isAlive()) {
            ...
            LOG.info("datanode statistics, freeBlocks {}, M {}, w {}, H {}, d {}", sumCount, stats.getM(), stats.getW(), stats.getH(), stats.getD());
