...
public class CrailBenchmark {
    ...
    // 对directStream，每个loop执行一次get(); 对BufferStream，执行一次get()
    ...
    double write(String filename, int size, int loop, int storageClass, int locationClass, boolean buffered, boolean
        ...
        fs.delete(filename, true).get().syncDir(); // 删除文件
        if (printInfo) {
    ...
    void testLocalDataWrite(String filename, int storageClass, int locationClass, boolean buffered, boolean skipDir) throws
        ...
        FileOutputStream fStream = new FileOutputStream(new File("Crail-IOTest-1.txt"));
        ...
        fStream = new FileOutputStream(new File("Crail-IOTest-2.txt"));
        ...
        fStream = new FileOutputStream(new File("Crail-IOTest-3.txt"));
        for (int loop = 1; loop <= 10; loop++) {
            double[] execTime = new double[loopTimes];
            double sumTime = 0.0;
            String TimeList = "";
