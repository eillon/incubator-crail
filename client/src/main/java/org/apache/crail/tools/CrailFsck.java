...
public class CrailFsck {
    ...
    public void createDirectory(String filename, int storageClass, int locationClass, double Mt) throws Exception {
        System.out.println("createDirectory, filename " + filename + ", storageClass " + storageClass + ", locationClass " + locationClass);
        CrailConfiguration conf = new CrailConfiguration();
        CrailStore fs = CrailStore.newInstance(conf);
        fs.create(filename, CrailNodeType.DIRECTORY, CrailStorageClass.get(storageClass), CrailLocationClass.get(locationClass), Mt, true).get().syncDir();
        fs.close();
    }
...
//				System.out.println("..........names " + host);
    ...
    public static void main(String[] args) throws Exception {
        ...
        Option MtOption = Option.builder("Mt").desc("block be stored time [1..n]").hasArg().build();
        ...
        if (type.equals("getLocations")) {
            ...
            fsck.createDirectory(filename, storageClass, locationClass, Mt);
        } else {
