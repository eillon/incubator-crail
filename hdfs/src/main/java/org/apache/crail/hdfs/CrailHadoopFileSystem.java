...
public class CrailHadoopFileSystem extends FileSystem {
    ...
    public FSDataOutputStream create(Path path, FsPermission permission,
        ...
        CrailFile fileInfo = null;
        try {
            fileInfo = dfs.create(path.toUri().getRawPath(), CrailNodeType.DATAFILE, CrailStorageClass.PARENT, CrailLocationClass.PARENT, 10, true).get().asFile();
        } catch (Exception e) {
    ...
    public boolean mkdirs(Path path, FsPermission permission) throws IOException {
        try {
            statistics.incrementWriteOps(1);
            CrailDirectory file = dfs.create(path.toUri().getRawPath(), CrailNodeType.DIRECTORY, CrailStorageClass.PARENT, CrailLocationClass.DEFAULT, 10, true).get().asDirectory();
            file.syncDir();
            return true;
        } catch (Exception e) {
            if (e.getMessage().contains(RpcErrors.messages[RpcErrors.ERR_PARENT_MISSING])) {
